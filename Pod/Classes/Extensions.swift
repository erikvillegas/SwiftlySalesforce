		...
		return Promise { seal in
			response(queue: nil) { rsp in
				if let error = rsp.error {
					...
					seal.reject(SerializationError.invalid(rsp, message: "Invalid response"))
				}
			}
		}
	}
			...
			responseJSON(queue: nil, options: options, completionHandler: { response in
				...
				case .success(let value):
					if let value = value as? [String: Any] {
						...
						seal.reject(SerializationError.invalid(value, message: "Invalid JSON dictionary response"))
					}
				case .failure(let error):
					seal.reject(error)
				}
